...
const OrderLayout: React.FC = () => {
  ...
  // 주문 관련 상태 및 액션 훅
  const { allTableOrders, setOrderStatus } = useBaseOrder();
  ...
  // 최대 페이지 수 계산
  const maxPageIndex = Math.ceil(allTableOrders.length / orderPerPage);

  // 현재 페이지에 표시할 주문 목록 슬라이싱
  const pagedOrders = useMemo(() => {
    const start = (pageIndex - 1) * orderPerPage;
    const end = start + orderPerPage;
    return allTableOrders.slice(start, end);
  }, [allTableOrders, pageIndex, orderPerPage]);
          ...
          <div className="flex justify-between items-center pb-[10px]">
            <div className="flex gap-[14px]">
              {Object.entries(TABLE_FILTER).map(([key, value]) => (
                <div
                  key={key}
                  className={`cursor-pointer hover:text-secondary-800 text-sm font-medium ${
                    selectedFilterMenu === key ? "font-bold text-secondary-800" : "text-secondary-600"
                  }`}
                  onClick={() => setSelectedFilterMenu(key)}
                >
                  {value}
                </div>
              ))}
          ...
          <div className="bg-primary-500-light-5 rounded-2xl p-6">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="w-[170px] h-[50px] bg-white rounded-2xl flex justify-center items-center gap-2">
                <div className="text-secondary-700-lighter text-[16px] font-bold">현재 주문 수</div>
                <div className="text-[20px] font-bold text-primary-800">{allTableOrders.length}개</div>
              </div>
            </div>
